...
import { RefreshCw, ShieldCheck, Activity, Scale, Receipt } from 'lucide-react';
...
export default function ArenaPage() {
  ...
  // Decision ledger: latest decision per question+outcome across recent runs
  const seen = new Set();
  const ledger = [];
  for (const r of runs) {
  ...
  const trades = runs.flatMap((r) => (r.executions || []).filter((e) => e.status !== 'dry_run' || !r.summary?.dryRun).map((e) => ({ ...e, runTs: r.timestamp }))).slice(0, 8);

  return (
    <AppShell
      title="Arena"
      subtitle="One agent core. Multiple venues. Every decision provable — live from Delphi."
      actions={
        <button
          onClick={() => fetchFeed(true)}
          className="flex items-center gap-1.5 border px-2.5 py-1.5 text-[12px] text-[var(--color-ink-muted)] hover:text-[var(--color-ink)]"
    ...
    >
      {error && runs.length === 0 && (
        <div className="mt-6 text-[13px] text-[var(--color-ink-muted)]">Waiting for the first cycle from the arena worker… {error}</div>
      )}

      {!latest && !error && (
        <div className="mt-6 text-[13px] text-[var(--color-ink-muted)]">Waiting for the first cycle from the arena worker…</div>
      )}
      ...
      {latest && (
        <>
          ...
          {/* ── Executions ──────────────────────────────────────────────── */}
          {trades.length > 0 && (
            <Section title="Executions" icon="receipt" aside={`${trades.length} recent`}>
              {trades.map((t, i) => (
                <Row key={i} first={i === 0}>
