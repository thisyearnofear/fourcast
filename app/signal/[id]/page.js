        ...
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6 space-y-5">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {signal.odds_efficiency && (
              <span
                className={`text-xs px-3 py-1.5 rounded-full font-medium ${efficiencyClass}`}
              >
                {signal.odds_efficiency}
              </span>
            )}
            {signal.venue && (
              <span className="text-xs px-3 py-1.5 rounded-full bg-slate-800/40 text-slate-400 border border-white/[0.06] font-medium">
                {signal.venue}
              </span>
            )}
            {signal.chain_origin && (
              <span className="text-xs px-3 py-1.5 rounded-full bg-blue-900/30 text-blue-300 border border-blue-600/20 font-medium">
                {signal.chain_origin}
              </span>
            )}
          </div>
