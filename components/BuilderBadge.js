...
export function BuilderBadge({ variant = 'compact', isNight = false }) {
  ...
  if (variant === 'compact') {
    return (
      <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-blue-500/20 text-blue-200 border border-blue-400/30`}>
  ...
  return (
    <div className={`flex flex-col gap-2 p-3 rounded-lg border bg-blue-500/10 border-blue-400/30`}>
      <div className="flex items-center gap-2">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center bg-blue-500/30`}>
      ...
      <a
        ...
        className={`text-xs font-medium underline text-blue-300 hover:text-blue-100`}
      >
        Track Performance →
      </a>
    </div>
