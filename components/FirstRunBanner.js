...
export default function FirstRunBanner({ searchQuery }) {
  ...
  return (
    <div className="mb-6 overflow-hidden rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.07] px-4 py-4 sm:px-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button
          type="button"
          onClick={dismiss}
          className="rounded-lg border border-white/10 bg-black/30 px-3 py-1.5 text-xs text-white/60 hover:text-white"
        >
          Got it
        </button>
      </div>
