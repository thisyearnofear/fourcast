...
// canvas-ui Glass — copied from https://canvasui.dev/docs/components/glass
...
//
// Usage: wrap any element to add a cursor-following glass lens that
// magnifies targets like a crystal ball. `data-glass-target` marks
// elements that should zoom in when hovered.
//
// Tasteful defaults for the dossier "What did the agent know?" block:
// size 140, low ior (1.35), quiet reflection, low aberration.
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        /* paint may fail when source canvas isn't visible; ignore */
      }
    };
  }
...
export function Glass({ children, className, style, priority = 7, motionBudget = true, ...options }) {
  ...
  // Motion budget: Glass is mid-cost (single lens, no per-frame simulation
  // work). When over budget, fall back to plain HTML so the wrapped content
  // still renders and the dossier/mandate panel stays functional.
  const { allowed: budgeted } = useMotionBudget(
    motionBudget ? `glass-${reactId}` : null,
    { priority },
  );
