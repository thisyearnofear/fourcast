...
export default function UnifiedConnect({ isNight = false, variant = 'header' }) {
 ...
 <div
 className={`${wasConnected ? 'w-[80px] h-[30px]' : 'w-[120px] h-[36px]'} bg-white/5`}
 aria-hidden="true"
 />
 </div>
 );
 ...
 {showDetails && (
 <div className={`absolute top-full right-0 mt-2 p-3 text-xs leading-relaxed z-50
 ...
 type="button"
 onClick={() => switchToArc()}
 className={`mt-2 w-full text-left text-[10px] py-1.5 px-2 bg-indigo-500/15 text-indigo-300`}
 >
