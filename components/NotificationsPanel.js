...
/**
 * NotificationsPanel — the alerts body, shared by /notifications (wrapper
 * route during transition) and the Signals "Alerts" tab. Wallet-gated;
 * renders its own no-wallet state so it embeds anywhere.
 */
export default function NotificationsPanel() {
  ...
  const fetchNotifications = useCallback(async (authHeaders) => {
    ...
    try {
      ...
      if (unreadData.success) setUnreadCount(unreadData.count || 0);
    } catch {
  ...
  useEffect(() => {
    ...
    if (cached) fetchNotifications(cached);
    else {
      setNeedsAuth(true);
      setIsLoading(false);
    }
  ...
  const parseData = (dataJson) => {
    try { return typeof dataJson === 'string' ? JSON.parse(dataJson) : dataJson; } catch { return {}; }
  };
  const typeIcons = { new_signal: '📊' };

  // No wallet connected — inline state so the panel embeds in a tab
  if (!address) {
    return (
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-4 px-1 sm:px-3">
        {unreadCount > 0 && (
          <>
      ...
      {needsAuth && !isLoading && (
        <div className="text-center py-16">
          ...
          <button onClick={handleVerify} className="mc-action mc-action--primary text-sm px-5 py-3">
      ...
      {!isLoading && !needsAuth && !error && notifications.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-xs text-ink-faint mb-6">Follow analysts to get notified when they publish.</p>
        </div>
      )}
